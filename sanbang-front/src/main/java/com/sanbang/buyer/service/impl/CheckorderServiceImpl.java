...
public class CheckorderServiceImpl implements CheckOrderService {
	...
	//生成合同地址
		private static final String PROCESS_CONTENT_URL="http://127.0.0.1:8080/website/certSign/forout/v2/processNewContractFast.do";
	...
	private Result checkStatisPriceForBuyer(ezs_orderform ezs_orderform, ezs_purchase_orderform purchaseOrder,
		...
		int status = Imblance_money.compareTo(income);

		String bz = "货款已付" + income + "元," + (status > 0 ? "应付/应退" + Imblance_money.subtract(income) + "元"
				: "应付/应退" + income.subtract(Imblance_money) + "元");
		checkorder.setMemo(bz);
		...
		if (status > 0) {
			...
			purchaseOrder.setOrder_status(93);
		}
	...
	public Result signContentProcess(Result result, String orderno) {
        try {
			...
			httpParam.addUrlParams(new BasicNameValuePair("memIds", buyer.getEzs_store().getNumber()+","+seller.getEzs_store().getNumber()));
			//操作合同用户id 
			httpParam.addUrlParams(new BasicNameValuePair("upmemId", "100002"));
			
			httpParam.addUrlParams(new BasicNameValuePair("temId", "7"));	
			
			httpParam.addUrlParams(new BasicNameValuePair("customerName", buyer.getEzs_store().getCompanyName()));
			//客户签章用户地址（供应商实名认证地址）
			httpParam.addUrlParams(new BasicNameValuePair("customerAddress", getaddressinfo(buyer.getEzs_store().getArea_id())+buyer.getEzs_store().getAddress()));
			//法定代表人（取实名认证数据）
			...
			if(null!=purchaseOrder){
				...
				httpParam.addUrlParams(new BasicNameValuePair("productAmountCase", NumberToCN.number2CNMontrayUnit(totalprice)));
				//交货时间  YYYY-MM-DD （预计交货时间）
				httpParam.addUrlParams(new BasicNameValuePair("productDelTime", Tools.date2Str(ezs_orderform.getEstimateTime(), "yyyy-MM-dd")));
				...
				for(int i=0;i<lists.size();i++){
					// 项目名称
					httpParam.addUrlParams(new BasicNameValuePair("qualityDetail["+i+"].name", lists.get(i).getEzs_qualityitem().getName()));
					// 实验方法
					httpParam.addUrlParams(new BasicNameValuePair("qualityDetail["+i+"].measure", lists.get(i).getEzs_qualityitem().getMeasure()));
					// 实验条件
					httpParam.addUrlParams(new BasicNameValuePair("qualityDetail["+i+"].term", lists.get(i).getEzs_qualityitem().getTerm()));
					// 单位
					httpParam.addUrlParams(new BasicNameValuePair("qualityDetail["+i+"].unit", lists.get(i).getEzs_qualityitem().getUnit()));
					// 产品规格
					httpParam.addUrlParams(new BasicNameValuePair("qualityDetail["+i+"].product_format", lists.get(i).getEzs_qualityitem().getProduct_format()));
					// 检测结果
					httpParam.addUrlParams(new BasicNameValuePair("qualityDetail["+i+"].q_result", lists.get(i).getEzs_qualityitem().getQ_result()));
				}
			 ...
			 * 卖家
			 */
			httpParam.addUrlParams(new BasicNameValuePair("sellercustomerName", seller.getEzs_store().getCompanyName()));
			//客户签章用户地址（供应商实名认证地址）
			httpParam.addUrlParams(new BasicNameValuePair("sellercustomerAddress", getaddressinfo(seller.getEzs_store().getArea_id())+seller.getEzs_store().getAddress()));
			//法定代表人（取实名认证数据）
			...
			//地址、电话
			httpParam.addUrlParams(new BasicNameValuePair("sellertaxpayeInfo", seller.getEzs_bill().getAddress()+seller.getEzs_bill().getPhone()));
			//开户行及账号
			httpParam.addUrlParams(new BasicNameValuePair("sellerbankAccount", seller.getEzs_bill().getBank()+seller.getEzs_bill().getNumber()));
			
			//电话（收款电话）
			httpParam.addUrlParams(new BasicNameValuePair("sellercustomerTel", seller.getEzs_userinfo().getTel()));
			//通讯地址（供应商收款地址）
			httpParam.addUrlParams(new BasicNameValuePair("sellercustomerTelAddress", seller.getEzs_store().getAddress()));
			//邮箱（注册邮箱）
			httpParam.addUrlParams(new BasicNameValuePair("sellercustomerEmail", seller.getEzs_userinfo().getEmail()));
			
			//post调用接口
			callBackRet= HttpRemoteRequestUtils.doPost(PROCESS_CONTENT_URL, httpParam);
			//获取合同的pdf地址
//		String pdfpath=(String) callBackRet.get("content");
			...
			if(content == null){
				...
				result.setSuccess(true);
				Map<String, Object> map=new HashMap<>();
				map.put("pdfurl", "http://test.ezaisheng.com/website/certSign/forout/showcontentpdf.do?orderid="+orderno);
			}
