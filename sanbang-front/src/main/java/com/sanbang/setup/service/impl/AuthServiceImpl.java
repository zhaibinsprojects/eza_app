...
public class AuthServiceImpl implements AuthService {
	...
	public Result saveComAuth(Result result, HttpServletRequest request, ezs_user upi, HttpServletResponse response) {
		...
		if (result.getSuccess()) {
			...
			uupi.getEzs_store().setAccountType(1);
			...
			if (null == gupi) {
				...
				boolean res = RedisUserSession.updateUserInfo(gupi.getAuthkey(), gupi,
	...
	public Result saveindivAuth(Result result, HttpServletRequest request, ezs_user upi, HttpServletResponse response) {
		...
		if (result.getSuccess()) {
			...
			if (null == gupi) {
				...
				gupi.getEzs_store().setAddress(address);
				gupi.getEzs_store().setPerson(persion);
				gupi.setTrueName(trueName);
				gupi.getEzs_store().setIdCardNum(idCardNum);
				gupi.getEzs_store().setAccount(account);
				uupi.getEzs_store().setAccountType(2);
				boolean res = RedisUserSession.updateUserInfo(gupi.getAuthkey(), gupi,
						Long.parseLong(redisuserkeyexpir));
